...
function obtenerPares(numArray1) {
    const arrayPares=[];
    for (let i=0; i<numArray1.length;i++){

        if(numArray1[i]%2===0){
            arrayPares.push(numArray1[i]);
        }
...
function pintarArray(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        resultado += i > 0 ? ', ' : ''; // Agrega una coma y espacio si no es el primer elemento
        resultado += array[i];
    }
...
function arrayMapi(array, funcion) {
    ...
    return array.map((elemento, indice) => funcion(elemento, indice));
}
...
function obtenerImpares(arrayNum) {
    let impares = [];
